...
const Productos: React.FC = () => {
  ...
  const handleCreatePromo = async (promoData: PromoInput) => {
    try {
      ...
      console.error(err);
      showNotification(
        err instanceof Error ? err.message : "Error al crear la promo",
        "error"
      );
      throw err;
    }
  ...
  const handleTogglePromo = async (promoId: number, active: boolean) => {
    try {
      await togglePromo(promoId, active);
      showNotification(
        active ? "Promo activada" : "Promo desactivada",
        "success"
      );
    } catch (err) {
      console.error(err);
      showNotification(
        err instanceof Error ? err.message : "Error al actualizar promo",
        "error"
      );
    }
  };
  ...
  const handleDeletePromo = async (promoId: number) => {
    const confirm = window.confirm(
      "¿Eliminar la promo seleccionada? Esta acción no se puede deshacer."
    );
    if (!confirm) return;
    try {
      ...
      console.error(err);
      showNotification(
        err instanceof Error ? err.message : "Error al eliminar promo",
        "error"
      );
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#181818] flex flex-col overflow-x-hidden w-full">
      <div className="flex-1 w-full p-4 sm:p-6 md:p-8 lg:max-w-7xl xl:max-w-[1800px] mx-auto max-w-full overflow-x-hidden">
        ...
        <ProductHeader
          isAdmin={isAdmin}
          onNewProduct={() => openFormModal()}
          onNewPromo={openPromoModal}
        />
